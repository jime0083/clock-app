...
exports.checkAlarms = (0, scheduler_1.onSchedule)({
    ...
    try {
        ...
        for (const doc of allUsersSnapshot.docs) {
            ...
            if (alarmTimeValue) {
                ...
                if (matched) {
                    ...
                    if (userData.lastAlarmOccurrence === occurrenceKey) {
                        console.log(`User ${userId}: Alarm already sent for ${occurrenceKey}`);
                    }
                    else if (!fcmToken) {
            ...
            if (!lastAlarmSentAt)
                continue;
            const lastSentMs = lastAlarmSentAt.toMillis?.() || new Date(lastAlarmSentAt).getTime();
            const elapsedMs = now.getTime() - lastSentMs;
            if (elapsedMs < 0 || elapsedMs > PENALTY_WINDOW_MS)
                continue;
            // These are reset to null on every initial send, so any value means
            // this occurrence was already handled
            if (userData.alarmAcknowledgedAt)
                continue;
            if (userData.squatCompletedAt)
                continue;
            if (userData.alarmFailedAt)
                continue;
            console.log(`User ${userId}: Re-alerting unacknowledged alarm ` +
                `(${Math.round(elapsedMs / 1000)}s elapsed)`);
            sendPromises.push(sendAlarmNotification(fcmToken, userId, language).then(() => undefined));
        }
...
async function postPenaltyTweetForUser(userId, userData) {
    ...
    const baseMessage = PENALTY_MESSAGES[language] ||
        PENALTY_MESSAGES.ja;
